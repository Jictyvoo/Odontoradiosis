...
function drawLandmark(div,landmarkName){const locations=global_points[landmarkName];let context=div.getContext('2d');context.beginPath();context.arc(locations.X,locations.Y,pointRadius,0,2*Math.PI);context.fillStyle='red';context.font=nameScale+"px Arial";context.fillText(landmarkName.match(/\(.+\)/),Math.floor(parseInt(locations.X)-40),Math.floor(parseInt(locations.Y)+40));context.fill();context.lineWidth=1;context.strokeStyle='#330005';context.stroke()}
...
mousePosition.x=currentPosition.x;mousePosition.y=currentPosition.y;drawPointCircle(curveName)}}}
function verifyMouseOnBoxVertex(relativeMouse,curveName){let boxVertex=getBoxDimensions(curveName,null,!0);const canvas=document.getElementById("bezier");const context=canvas.getContext("2d");let isOn=!1;[[boxVertex[0],boxVertex[1]],[boxVertex[0],boxVertex[1]+boxVertex[3]],[boxVertex[0]+boxVertex[2],boxVertex[1]],[boxVertex[0]+boxVertex[2],boxVertex[1]+boxVertex[3]]].forEach(function(element,index,array){drawCircle(context,element[0],element[1]);if(relativeMouse.x>=element[0]-pointRadius&&relativeMouse.x<=element[0]+pointRadius&&relativeMouse.y>=element[1]-pointRadius&&relativeMouse.y<=element[1]+pointRadius){isOn=!0}});return isOn}
