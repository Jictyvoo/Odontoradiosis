...
function loadJsonCurve(id){if(id&&id>0){const curveJson=curves_url.replace("%REPLACE%",id);fetch(curveJson).then(response=>{return response.json()}).then(data=>{all_curves=data}).then(()=>{draw_all_curves()})}}
