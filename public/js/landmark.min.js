...
function openImage(path,loadFunction){img=new Image();image_url=path;document.getElementById('stack-canvas').setAttribute("onmousedown","bezier_coordinate(event)");document.getElementById('stack-canvas').setAttribute("onmousemove","bezier_functions(event)");let ctx=document.getElementById('image');if(ctx.getContext){ctx=ctx.getContext('2d');ctx.canvas.width=1050;document.getElementById('landmarks').getContext('2d').canvas.width=ctx.canvas.width;document.getElementById('landmarks').getContext('2d').canvas.height=785;document.getElementById('bezier').getContext('2d').canvas.width=ctx.canvas.width;document.getElementById('bezier').getContext('2d').canvas.height=785;ctx.canvas.height=785;img.onload=function(){ctx.drawImage(img,0,0,ctx.canvas.width,ctx.canvas.height);ctx.fillStyle="rgba(1, 1, 1, 0)";if(loadFunction){loadFunction()}
loadJsonCurve()}}
img.src=path}
...
function drawLandmark(div,landmarkName){const locations=global_points[landmarkName];let context=div.getContext('2d');context.beginPath();const imageOffset=$("#image").offset();const imgOfLf=imageOffset.left;const imgOfTp=imageOffset.top;context.arc(locations.X-imgOfLf,locations.Y-imgOfTp,pointRadius,0,2*Math.PI);context.fillStyle='red';context.font="10px Arial";context.fillText(landmarkName,Math.floor(parseInt(locations.X)-imgOfLf-15),Math.floor(parseInt(locations.Y)-imgOfTp+15));context.fill();context.lineWidth=1;context.strokeStyle='#330005';context.stroke()}
...
function onChangeValue(){const imageDiv=document.getElementById('image');let filterValue=getValues();console.log(filterValue);imageDiv.setAttribute("style",filterValue)}
function reset(){brightness.value=100;contrast.value=100;grayscale.value=0;invert.value=0;onChangeValue()}
function drawBezier(x1,y1,cx1,cy1,cx2,cy2,x2,y2){const div=document.getElementById('bezier');let ctx=div.getContext('2d');const imageOffset=$("#image").offset();const imgOfLf=imageOffset.left;const imgOfTp=imageOffset.top;ctx.strokeStyle='#e3ed5c';ctx.moveTo(x1+imgOfLf,y1+imgOfTp);ctx.bezierCurveTo(cx1+imgOfLf,cy1+imgOfTp,cx2+imgOfLf,cy2+imgOfTp,x2+imgOfLf,y2+imgOfTp);ctx.stroke()}
function drawCircle(context,x,y){context.beginPath();const imageOffset=$("#image").offset();const imgOfLf=imageOffset.left;const imgOfTp=imageOffset.top;context.moveTo(x+imgOfLf,y+imgOfTp);context.arc(x+imgOfLf,y+imgOfTp,pointRadius,0,2*Math.PI);context.fillStyle='#184bed';context.fill();context.lineWidth=1;context.strokeStyle='#184bed';context.stroke()}
