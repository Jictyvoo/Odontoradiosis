...
function drawBezier(x1,y1,cx1,cy1,cx2,cy2,x2,y2){const div=document.getElementById('bezier');let ctx=div.getContext('2d');ctx.strokeStyle='#e3ed5c';ctx.moveTo(x1,y1);ctx.bezierCurveTo(cx1,cy1,cx2,cy2,x2,y2);ctx.stroke()}
...
function bezier_curve(currentCurve,recalculate){if(currentCurve!=null){currentCurve=currentCurve.replace(/ /g,"-").toLowerCase();draw_all_curves();if(all_curves[currentCurve]!=null){curveBox=getBoxDimensions(currentCurve,null,recalculate);let context=document.getElementById('bezier').getContext('2d');context.beginPath();context.rect(curveBox[0],curveBox[1],curveBox[2],curveBox[3]);context.stroke();drawBoxVertex(context)}}}
