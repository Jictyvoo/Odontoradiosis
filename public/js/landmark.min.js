class UsefulMethods{static canvasStyle(z_index=0,position="absolute",left=0,top=0){return `position: ${position}; left: ${left}; top: ${top}; z-index: ${z_index};`}
...
setStyle(id,newStyle){this.getCanvas(id).setAttribute("style",UsefulMethods.canvasStyle(this.layerSequence[id])+newStyle)}
...
onChangeValue(){let filterValue=this.getValues();this.canvasManager.setStyle("image",filterValue)}
