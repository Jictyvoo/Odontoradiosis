...
getCanvas(id){return this.allCanvas[id]}
getContext(id){return this.allCanvas[id].getContext("2d")}
...
openImage(path="",loadFunction=null){let imageObject=new Image();image_url=path;if(this.existentCanvas.image.getContext){const context=this.existentCanvas.image.getContext("2d");imageObject.onload=function(){context.canvas.width=this.width;context.canvas.height=this.height;["landmarks","bezier"].forEach(element=>{const temporaryContext=this.existentCanvas[element].getContext("2d");temporaryContext.canvas.width=context.canvas.width;temporaryContext.canvas.height=context.canvas.height});const cardCanvas=document.getElementById("card-canvas");if(cardCanvas){cardCanvas.setAttribute("style","height: "+context.canvas.height+"px")}
this.scaleDraw(document.getElementById("landmarks"));context.drawImage(imageObject,0,0,context.canvas.width,context.canvas.height);context.fillStyle="rgba(1, 1, 1, 0)";if(loadFunction){loadFunction()}}}
...
drawAllCurves(){this.canvasOdontoradiosis.clearCanvas("bezier");Object.keys(this.allCurves).forEach(function(element,index,array){this.allCurves[element].forEach(function(points,position,arr){if(position===0){this.canvas.drawBezier(this.canvas.getContext("bezier"),points[0],points[1],points[2],points[3],points[4],points[5],points[6],points[7],"#00e379")}else{let temporary=this.allCurves[element][position-1];this.canvas.drawBezier(this.canvas.getContext("bezier"),temporary[temporary.length-2],temporary[temporary.length-1],points[0],points[1],points[2],points[3],points[4],points[5],"#00e379")}})})}
...
onChangeValue(){const imageElement=this.canvas.getCanvas("image");let filterValue=getValues();imageElement.setAttribute("style",filterValue)}
...
redrawLandmark(){const landmarksCanvas=this.canvas.getCanvas("landmark");const context=landmarksCanvas.getContext("2d");context.clearRect(0,0,canvas.width,canvas.height);Object.keys(this.landmarks).forEach(function(element,index,array){this.drawLandmark(context,element)})}};class MainController{constructor(urls,canvasOdontoradiosis){this.urls=urls;this.tracingController=new TracingController(canvasOdontoradiosis);this.landmarksController=new LandmarksController(canvasOdontoradiosis)}
loadJsonLandmarks(id){if(id&&id>0){const landmarkJson=this.urls.landmarks.replace("%REPLACE%",id);fetch(landmarkJson).then(response=>{return response.json()}).then(data=>{this.landmarksController.setLandmarks(data)}).then(()=>{this.landmarksController.redrawLandmarks()})}}
loadJsonCurve(id){if(id&&id>0){const curveJson=this.urls.curves.replace("%REPLACE%",id);fetch(curveJson).then(response=>{return response.json()}).then(data=>{this.tracingController.setBezierPoints(data)}).then(()=>{this.tracingController.drawAllCurves()})}}
...
facial_analysis(){if(enable_draw_bezier){const glabela={x:all_curves["perfil-mole"][0][0],y:all_curves["perfil-mole"][0][1]};const subnasal={x:all_curves["perfil-mole"][1][4],y:all_curves["perfil-mole"][1][5]};const pogonion={x:all_curves["perfil-mole"][5][4],y:all_curves["perfil-mole"][5][5]};const div=document.getElementById("bezier");let ctx=div.getContext("2d");ctx.strokeStyle="#451c87";ctx.beginPath();ctx.moveTo(pogonion.x,pogonion.y);ctx.lineTo(subnasal.x,subnasal.y);ctx.lineTo(glabela.x,glabela.y);ctx.lineWidth=lineWidth*2;ctx.stroke();let toCalculate=normalizeValues(glabela,pogonion,subnasal);let angle=calculateAngle(toCalculate[0],toCalculate[1])*(subnasal.x>pogonion.x?-1:1);angle=(180*angle)/Math.PI;let td=document.getElementById("td-face_type");if(angle+5>=0&&angle+5<=10){td.innerHTML="Reto"}else if(angle<0){td.innerHTML="Concavo"}else{td.innerHTML="Convexo"}}}};class EventsOdontoradiosis{constructor(tracingController,imageEffects){this.tracingController=tracingController;this.imageEffects=imageEffects;this.isMouseDown=!1;this.isInsideBox=!1;this.isOnBoxVertex={isOn:!1,index:0};this.isOnCurvePoints=null;this.mousePosition={x:null,y:null};document.getElementById("stack-canvas").setAttribute("onmousedown","bezier_coordinate(event)");document.getElementById("stack-canvas").setAttribute("onmousemove","bezier_functions(event)")}
addEffectsEvent(){let elements=["contrast","brightness","invert","grayscale"];for(let i=0;i<elements.length;i++){document.getElementById(elements[i]).addEventListener("input",this.imageEffects.onChangeValue)}}
addCanvasEvents(){let curveSelect=document.getElementById("curvesId");curveSelect.addEventListener("input",function(){const selectedIndex=document.getElementById("curvesId").selectedIndex;const currentCurve=document.getElementById("curvesId").options[selectedIndex].text;this.tracingController.drawCurveBox(currentCurve);this.tracingController.drawPointCircle(currentCurve);if(currentCurve!=="Selecione"){document.getElementById("stack-canvas").style.cursor="move"}else{document.getElementById("stack-canvas").style.cursor="crosshair"}})}
generateMouseEvents(){const currentObject=this;document.onmousedown=function(){let hiddenForm=document.getElementById("current_image");const splicedSource=image_url.split("/");hiddenForm.setAttribute("value",splicedSource[splicedSource.length-2]+"/"+splicedSource[splicedSource.length-1]);currentObject.isMouseDown=!0};document.onmouseup=function(){currentObject.isMouseDown=!1;currentObject.isInsideBox=!1;currentObject.isOnBoxVertex={isOn:!1,index:0};currentObject.isOnCurvePoints=null;currentObject.mousePosition.x=null;currentObject.mousePosition.y=null}}
applyAllEvents(){this.addEffectsEvent();this.generateMouseEvents()}};let canvasOdontoradiosis=new CanvasOdontoradiosis(document.getElementById("stack-canvas"));let imageEffects=new ImageEffects(canvasOdontoradiosis);let mainController=new MainController({curves:curves_url,landmarks:landmarks_url,reference_images:reference_images_url},canvasOdontoradiosis);const eventsController=new EventsOdontoradiosis(mainController.tracingController,imageEffects);function image(path,id){global_points=[];global_effects=[];canvasOdontoradiosis.openImage(path,null,id);imageEffects.reset()}
