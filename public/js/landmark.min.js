...
function openImage(path,loadFunction){let img=new Image();image_url=path;document.getElementById('stack-canvas').setAttribute("onmousedown","bezier_coordinate(event)");document.getElementById('stack-canvas').setAttribute("onmousemove","bezier_functions(event)");let ctx=document.getElementById('image');if(ctx.getContext){ctx=ctx.getContext('2d');img.onload=function(){ctx.canvas.width=this.width;ctx.canvas.height=this.height;document.getElementById('landmarks').getContext('2d').canvas.width=ctx.canvas.width;document.getElementById('landmarks').getContext('2d').canvas.height=ctx.canvas.height;document.getElementById('bezier').getContext('2d').canvas.width=ctx.canvas.width;document.getElementById('bezier').getContext('2d').canvas.height=ctx.canvas.height;document.getElementById("card-canvas").setAttribute("style","height: "+ctx.canvas.height+"px");ctx.drawImage(img,0,0,ctx.canvas.width,ctx.canvas.height);ctx.fillStyle="rgba(1, 1, 1, 0)";if(loadFunction){loadFunction()}
...
const mousePosition=getMousePos(div,event);const offsets={left:$("#image").offset().left,top:$("#image").offset().top};global_points[currentPoint].X=mousePosition.x+offsets.left;global_points[currentPoint].Y=mousePosition.y+offsets.top;redrawLandmark(div);saveLandmarks(global_points)}}
function desfazer(){}
