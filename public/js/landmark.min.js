...
function openImage(path,loadFunction){let img=new Image();image_url=path;document.getElementById('stack-canvas').setAttribute("onmousedown","bezier_coordinate(event)");document.getElementById('stack-canvas').setAttribute("onmousemove","bezier_functions(event)");let ctx=document.getElementById('image');if(ctx.getContext){ctx=ctx.getContext('2d');img.onload=function(){ctx.canvas.width=this.width;ctx.canvas.height=this.height;document.getElementById('landmarks').getContext('2d').canvas.width=1050;document.getElementById('landmarks').getContext('2d').canvas.height=ctx.canvas.height;document.getElementById('bezier').getContext('2d').canvas.width=1050;document.getElementById('bezier').getContext('2d').canvas.height=ctx.canvas.height;document.getElementById("card-canvas").setAttribute("style","height: "+ctx.canvas.height+"px");ctx.drawImage(img,0,0,ctx.canvas.width,ctx.canvas.height);ctx.fillStyle="rgba(1, 1, 1, 0)";if(loadFunction){loadFunction()}
...
function drawLandmark(div,landmarkName){const locations=global_points[landmarkName];let context=div.getContext('2d');context.beginPath();context.arc(locations.X,locations.Y,pointRadius,0,2*Math.PI);context.fillStyle='red';context.font="10px Arial";context.fillText(landmarkName,Math.floor(parseInt(locations.X)-15),Math.floor(parseInt(locations.Y)+15));context.fill();context.lineWidth=1;context.strokeStyle='#330005';context.stroke()}
