...
img.src=path}
async function loadJsonCurve(){const splicedSource=image_url.split("/");const curveJson=curves_url.replace("%REPLACE%",splicedSource[splicedSource.length-2]+"@"+splicedSource[splicedSource.length-1]);fetch(curveJson).then(response=>{return response.json()}).then(data=>{all_curves=data}).then(()=>{draw_all_curves()})}
