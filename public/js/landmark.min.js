...
img.src=path}
function loadJsonLandmarks(){const splicedSource=image_url.replace(new RegExp('.jpg$'),'').split("/");const landmarkJson=landmarks_url.replace("%REPLACE%",splicedSource[splicedSource.length-2]+"@"+splicedSource[splicedSource.length-1]);fetch(landmarkJson).then(response=>{return response.json()}).then(data=>{global_points=data}).then(()=>{redrawLandmark(document.getElementById('landmarks'))})}
function loadJsonCurve(){const splicedSource=image_url.replace(new RegExp('.jpg$'),'').split("/");const curveJson=curves_url.replace("%REPLACE%",splicedSource[splicedSource.length-2]+"@"+splicedSource[splicedSource.length-1]);fetch(curveJson).then(response=>{return response.json()}).then(data=>{all_curves=data}).then(()=>{draw_all_curves()})}
...
function verifyMouseOnBoxVertex(relativeMouse,curveName){let boxVertex=getBoxDimensions(curveName,null,!0);let isOn=!1;let vertexIndex=0;[[boxVertex[0],boxVertex[1]],[boxVertex[0],boxVertex[1]+boxVertex[3]],[boxVertex[0]+boxVertex[2],boxVertex[1]],[boxVertex[0]+boxVertex[2],boxVertex[1]+boxVertex[3]]].forEach(function(element,index,array){if(relativeMouse.x>=element[0]-pointRadius&&relativeMouse.x<=element[0]+pointRadius&&relativeMouse.y>=element[1]-pointRadius&&relativeMouse.y<=element[1]+pointRadius){isOn=!0;vertexIndex=index}});return{isOn:isOn,index:vertexIndex}}
