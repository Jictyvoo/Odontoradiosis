const a__json={"landmark":"Ponto subespinhal (A)","accessed_curves":["maxila"],"routines":[["load_curve","maxila",null,"curvePoints"],["access_point",8,"curvePoints","foundedA"],["point_to_var","foundedA",null,{"x":"x","y":"y"}],["return","x","y",null]]};const ena__json={"landmark":"Espinha nasal anterior (ENA)","accessed_curves":["maxila"],"routines":[["load_curve","maxila",null,"curvePoints"],["access_point",7,"curvePoints","foundedENA"],["point_to_var","foundedENA",null,{"x":"x","y":"y"}],["return","x","y",null]]};const gnatio__json={"landmark":"Gnátio (Gn)","accessed_curves":["mandíbula"],"routines":[["load_curve","mandíbula",null,"curvePoints"],["access_point",8,"curvePoints","foundedGnatio"],["point_to_var","foundedGnatio",null,{"x":"x","y":"y"}],["return","x","y",null]]};const nasio__json={"landmark":"Násio (N)","accessed_curves":["sutura-fronto-nasal"],"routines":[["load_curve","sutura-fronto-nasal",null,"curvePoints"],["access_point",1,"curvePoints","foundedNasio"],["point_to_var","foundedNasio",null,{"x":"x","y":"y"}],["return","x","y",null]]};const sela__json={"landmark":"Sela (S)","accessed_curves":["sela-túrcica"],"routines":[["load_curve","sela-túrcica",null,"curvePoints"],["access_point",4,"curvePoints","p1"],["access_point",5,"curvePoints","p2"],["access_point",6,"curvePoints","p3"],["access_point",7,"curvePoints","p1"],["average",["p1","p2","p3","p4"],null,"result"],["point_to_var","result",null,{"x":"x","y":"y"}],["return","x","y",null]]};class OdontoradiosisKeeper{constructor(){this.isMouseDown=!1;this.isInsideBox=!1;this.isOnBoxVertex={isOn:!1,index:0};this.isOnCurvePoints=null;this.mousePosition={x:null,y:null};this.isCurveFunction=!1}};class UsefulMethods{static canvasStyle(z_index=0,position="absolute",left=0,top=0){return `position: ${position}; left: ${left}; top: ${top}; z-index: ${z_index};`}
...
getCurve(curveId=""){if(this.curveExists(curveId)){return this.bezierPoints[curveId]}
return null}
...
setLandmark(name,value={X:0,Y:0}){this.landmarks[name]=value}
...
applyAllEvents(){this.addEffectsEvent();this.addCanvasInputEvents();this.generateMouseEvents()}};class SemiautomaticLandmarks{constructor(routinesDescription,tracingController,landmarksController){this.routinesDescription=routinesDescription;this.tracingController=tracingController;this.landmarksController=landmarksController;const symbolTable=[];const helpVariables={accessed_curves:[],landmarkName:""};this.symbolTable=symbolTable;this.helpVariables=helpVariables;const getParamValue=function(parameter){return symbolTable[parameter]!=null?symbolTable[parameter]:parameter};this.preFunctions={load_curve:function(param_1,param_2,resultName){const loadedCurves=[tracingController.getCurve(param_1)];if(param_2!=null){loadedCurves.push(tracingController.getCurve(param_2));symbolTable[resultName+"_1"]=loadedCurves[0];symbolTable[resultName+"_2"]=loadedCurves[1]}else{symbolTable[resultName]=loadedCurves[0]}
return loadedCurves},access_point:function(param_1,param_2,resultName){const accessed={x:0,y:0};let counter=0;for(let index=0;index<symbolTable[param_2].length;index++){const element=symbolTable[param_2][index];for(let subindex=1;subindex<element.length;subindex+=2){counter++;if(counter==param_1){accessed.x=element[subindex-1];accessed.y=element[subindex]}}}
symbolTable[resultName]=accessed;return accessed},point_to_var:function(param_1,param_2,resultName){const toVar=symbolTable[param_1];symbolTable[resultName.x]=toVar.x;symbolTable[resultName.y]=toVar.y},add:function(param_1,param_2,resultName){const value_1=getParamValue(param_1);const value_2=getParamValue(param_2);symbolTable[resultName]=value_1+value_2;return symbolTable[resultName]},sub:function(param_1,param_2,resultName){const value_1=getParamValue(param_1);const value_2=getParamValue(param_2);symbolTable[resultName]=value_1-value_2;return symbolTable[resultName]},div:function(param_1,param_2,resultName){const value_1=getParamValue(param_1);const value_2=getParamValue(param_2);symbolTable[resultName]=value_1/value_2;return symbolTable[resultName]},mul:function(param_1,param_2,resultName){const value_1=getParamValue(param_1);const value_2=getParamValue(param_2);symbolTable[resultName]=value_1*value_2;return symbolTable[resultName]},mod:function(param_1,param_2,resultName){const value_1=getParamValue(param_1);const value_2=getParamValue(param_2);symbolTable[resultName]=value_1%value_2;return symbolTable[resultName]},average:function(param_1,param_2,resultName){let average=0;const total=param_1.length;for(let index=0;index<param_1.length;index++){average+=param_1[index]}
symbolTable[resultName]=average/total;return symbolTable[resultName]},return:function(param_1,param_2,resultName){const result={X:symbolTable[param_1],Y:symbolTable[param_2]};landmarksController.setLandmark(helpVariables.landmarkName,result);landmarksController.redrawLandmarks();return result}}}
generateButtonEvent(){const self=this;document.getElementById("semiautomatic_button").onclick=function(){self.start.call(self)}}
start(){for(let index=0;index<this.routinesDescription.length;index++){const currentRoutine=this.routinesDescription[index];this.helpVariables.landmarkName=currentRoutine.landmark;this.helpVariables.accessed_curves=currentRoutine.accessed_curves;for(let position=0;position<currentRoutine.routines.length;position++){const element=currentRoutine.routines[position];this.preFunctions[element[0]](element[1],element[2],element[3])}}
return!0}};const scaleManager=new ScaleManager();const canvasOdontoradiosis=new CanvasOdontoradiosis(document.getElementById("stack-canvas"),scaleManager,{image:0,bezier:1,landmarks:2});const imageEffects=new ImageEffects(canvasOdontoradiosis);const infoKeeper=new OdontoradiosisKeeper();const mainController=new MainController({image:"",curves:curves_url,landmarks:landmarks_url,reference_images:reference_images_url},canvasOdontoradiosis,scaleManager,infoKeeper);const semiautomaticLandmarks=new SemiautomaticLandmarks([a__json,ena__json,gnatio__json,nasio__json,sela__json],mainController.tracingController,mainController.landmarksController);const eventsController=new EventsOdontoradiosis(mainController,infoKeeper,imageEffects);function openImage(path="",id=-1){mainController.tracingController.setBezierPoints([]);mainController.setUrl("image",path);canvasOdontoradiosis.openImage(path,function(){mainController.loadJsonCurve(id);mainController.loadJsonLandmarks(id)},id);imageEffects.reset()}
window.onload=function(){eventsController.applyAllEvents();semiautomaticLandmarks.generateButtonEvent()}