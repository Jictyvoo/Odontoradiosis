...
if ($DatabaseController) {
	$fkImgs = $DatabaseController->generateDAO("img_save")->select(array("fk_id_img"), "id_doctor= " . $_SESSION['logged']->getUserID());
	$paths = array();
	for ($i = 0; $i < sizeof($fkImgs); $i++) {
		$array = $DatabaseController->generateDAO("imagem")->select(array("caminho"), "id_imagem=" . $fkImgs[$i][0]);
